...
function addAdventureDetailsToDOM(adventure) {
  ...
  adventure.images.map((image)=>{
    ...
    imgElem.className= "col-lg-12"
    imgElem.innerHTML=`<img src=${image} alt="" srcset="" class= activity-card-image pb-3 pb-md-0 />`
    document.getElementById("photo-gallery").append(imgElem)
  })
...
images.map((image, idx)=>{
  let imgElem= document.createElement("div")
  imgElem.className= `carousel-item ${idx===0?"active":""}`
  imgElem.innerHTML=`<img src=${image} alt="" srcset="" class= activity-card-image pb-3 pb-md-0 />`
  document.getElementById("carousel-inner").append(imgElem)
})
...
function showBannerIfAlreadyReserved(adventure) {
  // TODO: MODULE_RESERVATIONS
  // 1. If user has already reserved this adventure, show the reserved-banner, else don'
